...
int main(int argc, char *argv[]) {
	if(argc < 2) {
		cout << "Usage: ./trainsom [options] -TrainingMatrix <Training Matrix File Location> -Output <Output File Location>" <<endl;
		return 0;
	}
	...
	while(getline(trainingFile,line)) {
		...
		if(dimension == -1) dimension = fields.size()-1;
		vector<double> temp;
	...
	//cout<<"Setting up input"<<endl;
	//cout<<floor(dataKeys.size()*.01)<<' '<<dimension<<endl;
	Eigen::MatrixXd InputMap = Eigen::MatrixXd::Zero(floor(dataKeys.size()*.01)+1,dimension);
	//cout<<InputMap.rows()<<' '<<dataKeys.size()*.01<<endl;
	for(int i = 0; i < dataKeys.size()*.01; i++) {
		for(int j = 0; j < dimension; j++) {
			InputMap(i,j)=dataMap[dataKeys[i]][j];
		}
	}
	cout<<"subbing mean"<<endl;
	for(int i = 0; i < InputMap.cols(); i++) {
		double mean = (InputMap.col(i).sum())/InputMap.rows();
		Eigen::VectorXd meanVector = Eigen::VectorXd::Constant(InputMap.rows(), mean);
		InputMap.col(i) -= meanVector;
	}
	Eigen::MatrixXd Covariance = Eigen::MatrixXd::Zero(InputMap.rows(),InputMap.rows());
	Covariance = (1 / (double)dimension)*InputMap*InputMap.transpose();

	Eigen::EigenSolver<Eigen::MatrixXd> m_solve(Covariance);
	Eigen::VectorXd eigenvalues = Eigen::VectorXd::Zero(InputMap.rows());
	eigenvalues = m_solve.eigenvalues().real();

	Eigen::MatrixXd eigenVectors = Eigen::MatrixXd::Zero(dimension, InputMap.rows());
	eigenVectors = m_solve.eigenvectors().real();

	vector<pair<double, int> > pi;
	for(int i = 0; i < InputMap.rows(); i++) {
	...
	i=pi.size()-i;
	cout<<i<<endl;
	float ratio = (totalsave-pi[pi.size()-1].first)/totalsave;
	cout<<ratio<<endl;
	int rows = floor(sqrt(i*20/(1+ratio)));
	int cols = rows*(1+ratio);
	cout<<rows<<' '<<cols<<endl;
	ofstream Outfile(somFile.c_str());
	Outfile<<rows<<'\t'<<cols<<endl;
	Outfile.close();
	/*int i = pi.size()-1;
	int howMany = i - dimension;
	for(; i > howMany; i--) {
